...
const SubNavbar = () => {
    ...
    return (
        ...
        <div className="flex items-center gap-2">
            <button onClick={() => navigate('/auth/login')} className="rounded-lg px-4 py-2 text-sm font-medium text-zinc-800 hover:bg-zinc-100 dark:text-zinc-100 dark:hover:bg-zinc-900">
                Log in
            </button>
            <button onClick={() => navigate('/auth/signup')} className="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-700">
                Sign up
            </button>
...
const IsAdmin = () => {
    return (
        <div className='flex items-center gap-4'>
            <h1>Admin</h1>
            <button onClick={handleLogout} className='rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-700'>Logout</button>
        </div>
    )
}
        ...
        <header className="sticky top-0 z-40 border-b border-zinc-200 bg-white/80 backdrop-blur dark:border-zinc-800 dark:bg-zinc-950/70">
            <div className="mx-auto flex items-center justify-between px-12 py-3">
                <Link to="/" className="flex items-center gap-2 text-lg font-semibold">
                    <span className="inline-flex h-8 w-8 items-center justify-center rounded-md bg-indigo-600 text-white">🎟️</span>
                    <span>Eventix</span>
                </Link>

                <nav className="hidden items-center gap-6 text-sm text-zinc-600 dark:text-zinc-300 sm:flex">
                    <a href="#events" className="hover:text-zinc-900 dark:hover:text-zinc-100">Events</a>
                    <a href="#" className="hover:text-zinc-900 dark:hover:text-zinc-100">Clubs</a>
